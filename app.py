...
attack_active = False
current_attack = None
packet_history = []  # Store recent packets for analysis
MAX_HISTORY = 100
attack_thresholds = {
    'DDoS': 50,  # Packets per second
    'PortScan': 10,  # Unique ports per second
    'SQLInjection': 5,  # Malicious packets with specific size range (200-1000)
    'XSS': 5,  # Malicious packets with size range (300-1500)
    'Malware': 20  # Large packets per second (500-3000)
}
...
def monitor_network():
    global sniffing, packet_history
    while sniffing:
        def packet_handler(packet):
            global packet_history, current_attack
            if IP in packet and sniffing:
                try:
                    ...
                    attack = detect_attack(packet_history[-50:])  # Check last 50 packets
                    if attack and attack != current_attack:
                        ...
                        socketio.emit('attack_highlight', {'attack': null})
                        current_attack = None
...
def stop_monitoring():
    global sniffing, current_attack
    sniffing = False
    if current_attack:
        socketio.emit('attack_highlight', {'attack': null})
        current_attack = None
    ...
    if not attack_active:
        ...
        def attack_thread():
            ...
            while (datetime.now() - start_time).total_seconds() < attack_duration and attack_active:
                ...
                socketio.emit('new_packet', pkt_info)
                packet_count += 1
            emit('status_update', {'status': f'{attack_type} Attack Ended'})
            socketio.emit('attack_highlight', {'attack': null})
            attack_active = False
            current_attack = None
